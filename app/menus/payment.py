...
from app.client.engsel import get_transaction_history from app.menus.util import clear_screen
...
def show_transaction_history(api_key, tokens):
    ...
    while in_transaction_menu:                                clear_screen()
        ...
        if len(history) == 0:
            ...
            table.add_column("Date", style="cyan")                table.add_column("Status", style="dim")
            ...
            for idx, transaction in enumerate(history, start=1):
                ...
                formatted_time = dt_jakarta.strftime("%d %b %Y %H:%M")
                                                                      status_color = "green" if transaction['status'] == "SUCCESS" else "red"                                     status_display = f"[{status_color}]{transaction['status']}[/]"                              
                table.add_row(
                    str(idx),
                    f"{transaction['title']}\n[dim]{transaction['payment_method_label']} - {transaction['price']}[/]",
                    formatted_time,                                       status_display
                )

            print_cyber_panel(table, title="RIWAYAT TRANSAKSI")
                                                              # Option
        console.print("[dim]0. Refresh | 00. Kembali ke Menu Utama[/]")
        choice = cyber_input("Pilih opsi")                    if choice == "0":
            continue
        elif choice == "00":
