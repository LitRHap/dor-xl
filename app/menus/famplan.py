        ...
        if choice == "1":
            ...
            try:
                ...
                if members[slot_idx_int - 1].get("msisdn") != "":
                    ...
                    pause()
                    return
                                                                      family_member_id = members[slot_idx_int - 1]["family_member_id"]                                            slot_id = members[slot_idx_int - 1]["slot_id"]
                ...
                if validation_res.get("status").lower() != "success":
                    ...
                    console.print_json(data=validation_res)                                                                     pause()
                    return
                console.print("[neon_green]MSISDN validation successful.[/]")

                target_family_plan_role = validation_res["data"].get("family_plan_role", "")
                if target_family_plan_role != "NO_ROLE":                                                                        console.print(f"[warning]{target_msisdn} is already part of another family plan with role {target_family_plan_role}.[/]")                                         pause()
                    return
            ...
            try:
                slot_idx_int = int(slot_num)                          if slot_idx_int < 1 or slot_idx_int > len(members):
                    console.print("[error]Invalid slot number.[/]")
                    pause()                                               return

                member = members[slot_idx_int - 1]                    if member.get("msisdn") == "":
                    console.print("[warning]Selected slot is already empty.[/]")
                    pause()
                    return                            
                is_continue = cyber_input(f"Are you sure you want to remove member {member.get('msisdn')} from slot {slot_idx_int}? (y/n)").strip().lower()                       if is_continue != "y":
                    console.print("[warning]Operation cancelled by user.[/]")
                    pause()
                    return                            
                family_member_id = member["family_member_id"]                                               
                with loading_animation("Removing member..."):
                    res = remove_member(                                      api_key,
                        tokens,
                        family_member_id,
                    )
                ...
                console.print_json(data=res)                      except ValueError:
                console.print("[error]Invalid input. Please enter a valid slot number.[/]")
            pause()
            ...
            try:
                ...
                if len(parts) < 3:
                    raise ValueError                  
                slot_num = parts[1]                                   new_quota_mb = parts[2]
                ...
                if member.get("msisdn") == "":                            console.print("[warning]Selected slot is empty. Cannot set quota limit.[/]")
                    pause()
                    return
                ...
                console.print("[error]Invalid input. Usage: limit <slot> <mb>[/]")                                      pause()
        elif choice == "00":
