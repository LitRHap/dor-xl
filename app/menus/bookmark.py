...
def show_bookmark_menu():
    ...
    in_bookmark_menu = True                               while in_bookmark_menu:
        ...
        bookmarks = BookmarkInstance.get_bookmarks()          if not bookmarks or len(bookmarks) == 0:
        ...
        table.add_column("Family", style="bold white")        table.add_column("Variant", style="cyan")
        ...
        if choice == "00":
            ...
            if del_choice.isdigit() and 1 <= int(del_choice) <= len(bookmarks):
                ...
                BookmarkInstance.remove_bookmark(                         del_bm["family_code"],
                    del_bm["is_enterprise"],                              del_bm["variant_name"],
                    del_bm["order"],
                )
                console.print("[neon_green]Bookmark deleted.[/]")                                                           pause()
            else:
                console.print("[error]Input tidak valid. Silahkan coba lagi.[/]")
                pause()
            continue
        if choice.isdigit() and 1 <= int(choice) <= len(bookmarks):                                                     selected_bm = bookmarks[int(choice) - 1]
            ...
            option_code = None                                    for variant in package_variants:
                if variant["name"] == selected_bm["variant_name"]:
                    selected_variant = variant        
                    package_options = selected_variant["package_options"]
                    for option in package_options:                            if option["order"] == selected_bm["order"]:
