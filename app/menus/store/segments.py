...
from app.menus.util import clear_screen, pause        from app.service.auth import AuthInstance
...
def show_store_segments_menu(is_enterprise: bool = False):
    ...
    while in_store_segments_menu:                             api_key = AuthInstance.api_key
        tokens = AuthInstance.get_active_tokens()     
        with loading_animation("Fetching store segments..."):                                                           segments_res = get_segments(api_key, tokens, is_enterprise)                                     
        if not segments_res:
            console.print("[warning]No segments found.[/]")
            in_store_segments_menu = False                        continue

        segments = segments_res.get("data", {}).get("store_segments", [])                                   
        clear_screen()
                                                              packages = {}

        for i, segment in enumerate(segments):                    name = segment.get("title", "N/A")
            banners = segment.get("banners", [])      
            letter = chr(65 + i)  # Convert 0 -> A, 1 -> B, etc.
                                                                  table = Table(show_header=True, header_style="neon_pink", box=None, padding=(0, 1))                         table.add_column("ID", style="neon_green", justify="right", width=4)                                        table.add_column("Package Name", style="bold white")
            ...
            table.add_column("Validity", style="cyan")
                ...
                action_type = banner.get("action_type", "")
                    ...
                    "action_param": action_param,                         "action_type": action_type
                }                                     
                table.add_row(
                    f"{letter}{j + 1}",
                    f"{family_name} - {title}",                           f"Rp{discounted_price}",
                    validity
                )                                     
        ...
        action_param = selected_pkg["action_param"]           action_type = selected_pkg["action_type"]
