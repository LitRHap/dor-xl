from app.client.store.search import get_family_list, get_store_packages                                     from app.menus.package import get_packages_by_family, show_package_details                                  from app.menus.util import clear_screen, pause
...
def show_family_list_menu(
    ...
    while in_family_list_menu:
        ...
        if not family_list_res:                                   console.print("[warning]No family list found.[/]")
            in_family_list_menu = False                           continue

        family_list = family_list_res.get("data", {}).get("results", [])

        clear_screen()
                                                              table = Table(show_header=True, header_style="neon_pink", box=None)
        table.add_column("No", style="neon_green", justify="right", width=4)                                        table.add_column("Family Name", style="bold white")
        ...
        if choice == "00":                                        in_family_list_menu = False
            continue
        ...
        else:                                                      console.print("[error]Invalid choice.[/]")
             pause()
...
def show_store_packages_menu(
    ...
    while in_store_packages_menu:
        ...
        if not store_packages_res:                                console.print("[warning]No store packages found.[/]")
        ...
        clear_screen()                                
        packages = {}                                 
        table = Table(show_header=True, header_style="neon_pink", box=None)                                         table.add_column("No", style="neon_green", justify="right", width=4)
        table.add_column("Package", style="bold white")
        table.add_column("Price", style="yellow")
        table.add_column("Validity", style="cyan")    
        for i, package in enumerate(store_packages):              title = package.get("title", "N/A")

            original_price = package.get("original_price", 0)
            discounted_price = package.get("discounted_price", 0)                                           
            price = original_price                                if discounted_price > 0:
        ...
        if choice == "00":                                        in_store_packages_menu = False
            continue                                                                                                if choice in packages:
